...
namespace Prototype
{
    ...
    public class PlayerController : MonoBehaviour
    {
        ...
        bool
            jumpKeyWasPressed; // Indicates if the jump key was pressed since the last reset, used to detect jump initiation

        bool
            jumpKeyWasLetGo; // Indicates if the jump key was released since it was last pressed, used to detect when to stop jumping
        ...
        void SetupStateMachine()
        {
            ...
            stateMachine.AddTransition(grounded, rising, new FunctionPredicate(() => IsRising()));
            stateMachine.AddTransition(grounded, sliding,
                new FunctionPredicate(() => mover.IsGrounded() && IsGroundTooSteep()));
            // stateMachine.AddTransition(grounded, falling, new FunctionPredicate(() => !mover.IsGrounded()));
            stateMachine.AddTransition(grounded, jumping,
                new FunctionPredicate(() => (jumpKeyIsPressed || jumpKeyWasPressed) && !jumpInputIsLocked));

            stateMachine.AddTransition(falling, rising, new FunctionPredicate(() => IsRising()));
            stateMachine.AddTransition(falling, grounded,
                new FunctionPredicate(() => mover.IsGrounded() && !IsGroundTooSteep()));
            stateMachine.AddTransition(falling, sliding,
                new FunctionPredicate(() => mover.IsGrounded() && IsGroundTooSteep()));

            stateMachine.AddTransition(sliding, rising, new FunctionPredicate(() => IsRising()));
            stateMachine.AddTransition(sliding, falling, new FunctionPredicate(() => !mover.IsGrounded()));
            stateMachine.AddTransition(sliding, grounded,
                new FunctionPredicate(() => mover.IsGrounded() && !IsGroundTooSteep()));

            stateMachine.AddTransition(rising, grounded,
                new FunctionPredicate(() => mover.IsGrounded() && !IsGroundTooSteep()));
            stateMachine.AddTransition(rising, sliding,
                new FunctionPredicate(() => mover.IsGrounded() && IsGroundTooSteep()));
            stateMachine.AddTransition(rising, falling, new FunctionPredicate(() => IsFalling()));
            stateMachine.AddTransition(rising, falling,
                new FunctionPredicate(() => ceilingDetector != null && ceilingDetector.HitCeiling()));

            stateMachine.AddTransition(jumping, rising,
                new FunctionPredicate(() => jumpTimer.IsFinished || jumpKeyWasLetGo));
            stateMachine.AddTransition(jumping, falling,
                new FunctionPredicate(() => ceilingDetector != null && ceilingDetector.HitCeiling()));
