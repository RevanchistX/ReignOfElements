...
namespace AdvancedController
{
    public class CameraController : MonoBehaviour
    {
        ...
        [Range(0f, 90f)]
        public float upperVerticalLimit = 35f;

        [Range(0f, 90f)]
        public float lowerVerticalLimit = 35f;
        ...
        // void ShouldRotateCamera(Vector2 position, bool performed)
        void ShouldRotateCamera(bool performed)
        {
            // Debug.Log($"shouldrotatecamera: {performed}");
            if (!performed) return;
            // Debug.Log($"performed {input.LookDirection}");
            RotateCamera(input.LookDirection.x, input.LookDirection.y);
        }
